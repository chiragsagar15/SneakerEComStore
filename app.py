...
mongodb_client = MongoClient(
    f"mongodb+srv://chiragsagar:Chiragsagar15@cluster0.9jy65.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
)
try:
...
# Route for user profile page with GET and POST methods
...
# Route for admin dashboard with pagination and statistics
...
# Route for updating order status in the admin dashboard
...
# Route for adding sample products to the database
...
# Route for home page displaying limited products
@app.route("/")
def index():
...
# Route for displaying user's cart with total price
@app.route("/cart")
@login_required
def cart():
...
# Route for checkout, with optional product_id parameter
...
# Route for confirming the order and storing order data
...
# Route for displaying user's order history
...
# Route for deleting an order in the admin dashboard
